# File is written by Evelyn Beck
...
def read_img(ser, width, height, bytes_per_pixel=2, verbose=False):
    ...
    for i in range(height):
        ...
        for j in range(width):
            ...
            input = ser.read(bytes_per_pixel)
...
def save_numpy(img_data, filename):
    ...
    print(
        f'saving image as numpy array of shape {img_array.shape} to', filename)
...
def main():
    ...
    parser.add_argument('--npy-output', metavar='FILE', type=str,
                        help='output image in native color as numpy array to FILE')
    parser.add_argument('--img-output', metavar='FILE',
                        type=str, help='output image in RGB888 form to FILE')
    ...
    if npy_output is None and img_output is None:
        parser.error(
            'at least one of --npy-output or --img-output must be specified')

    with serial.Serial(port, baud) as ser:
        ser.write(b'c')  # send 'c' to start capture
        img_data = read_img(ser, width, height,
                            bytes_per_pixel, verbose=verbose)
