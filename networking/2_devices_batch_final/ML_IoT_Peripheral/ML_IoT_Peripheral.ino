...
#define NBR_BATCHES_ITER \
    (DYN_NBR_WEIGHTS / BLE_NBR_WEIGHTS)  // Max 255 before overload
...
BLEService weightsService(
    "19B10000-E8F2-537E-4F6C-D104768A1214");  // BLE Weights Service

BLECharacteristic readCharacteristic("19B10001-E8F2-537E-4F6C-D104768A1214",
                                     BLERead | BLEIndicate, sizeof(bleData));
BLECharacteristic writeCharacteristic("19B10001-E8F2-537E-4F6C-D104768A1215",
                                      BLEWrite, sizeof(bleData));
...
void aggregate_weights() {
    ...
    PRINT_BLE(false);
#endif
    // Read input, does not need to be stored in a buffer, just use .value() or
    // [] to access offset; weightsData = *((weights_data_t
    // *)writeCharacteristic.value()); But for now we copied the data into
    // "bleData" before call to this function, might optimize later
    for (int i = 0; i < BLE_NBR_WEIGHTS; i++) {
...
void loop() {
    ...
    if (writeCharacteristic.written() || timeout) {
        ...
        if (receivedTurn == 0 || timeout) {
            ...
            // Have all weights for this iteration been aggregated? I.e.
            // iteration finished?
            if (turn == NBR_CENTRALS && batch_id == NBR_BATCHES_ITER - 1 ||
                timeout) {
                turn = 1;  // Start over
                bleData.turn = turn;
                // Beginning of new iteration, distribute weights from previous
                // iteration and tell node 1 to send training result
                ...
                for (int i = 0; i < NBR_BATCHES_ITER; i++) {
                    ...
                    readCharacteristic.writeValue((byte *)&bleData,
                                                  sizeof(bleData));
#if DEBUG
                    PRINT_BLE(true);
#endif
                ...
                // Calculate our contribution to next iteration, while others
                // also are calculating But dont if timeout, to avoid unequal
                // sample size
                if (!timeout) do_training();
            } else if (bleData.batch_id == NBR_BATCHES_ITER - 1) {
                bleData.turn = ++turn;
                // Tell next node to send its weights, note: no need to
